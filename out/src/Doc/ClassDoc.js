...
var _UtilNamingUtilJs = require('../Util/NamingUtil.js');

var _UtilNamingUtilJs2 = _interopRequireDefault(_UtilNamingUtilJs);
...
var ClassDoc = (function (_AbstractDoc) {
  ...
  _createClass(ClassDoc, [{
    ...
    key: 'findReactPropTypesNode',
    value: function findReactPropTypesNode() {
      var ptNode = null;
      _UtilASTUtilJs2['default'].traverse(this._node, function (node) {
    ...
    key: 'findReactPropIsRequired',
    value: function findReactPropIsRequired(ptNode) {
      var isRequired = false;
      _UtilASTUtilJs2['default'].traverse(ptNode, function (n) {
    ...
    key: 'findReactPropType',
    value: function findReactPropType(ptNode) {
      var nodeVal = ptNode.value;
      ...
      if (nodeVal.arguments && nodeVal.arguments.length > 0) {
        var isArray = ptNode.value.callee.property.name === 'arrayOf';
        var firstArg = nodeVal.arguments[0];
        if (firstArg.properties) {
          var strProps = firstArg.properties.map(function (p) {
            return p.key.name;
          });
          if (isArray) {
        ...
        if (firstArg.property) {
          var propName = firstArg.property.name;
          if (isArray) {
    ...
    key: 'parsePropType',
    value: function parsePropType(node) {
      var result = { name: node.key.name };
      result.isRequired = this.findReactPropIsRequired(node);
      result.type = this.findReactPropType(node) || 'unknown';
      return result;
    }
  }, {
    key: 'parseReactPropTypes',
    value: function parseReactPropTypes() {
      var propTypeNode = this.findReactPropTypesNode();
    ...
    value: function _name() {
      ...
      if (this._node.id) {
        ...
        this._value.name = _UtilNamingUtilJs2['default'].filePathToName(this._pathResolver.filePath);
      }
    }
