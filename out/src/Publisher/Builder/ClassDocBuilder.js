...
function getReactPropTypesExample(className, props) {
  ...
  result += props.reduce(function (acc, val) {
    var required = val.isRequired ? '!required' : '';
  ...
  _createClass(ClassDocBuilder, [{
    ...
    value: function _buildClassDoc(doc) {
      ...
      ice.into('instanceDocs', instanceDocs, function (instanceDocs, ice) {
        ice.loop('instanceDoc', instanceDocs, function (i, instanceDoc, ice) {
          ice.load('instanceDoc', _this._buildDocLinkHTML(instanceDoc.longname));
        });
      });

      ice.into('exampleDocs', doc.examples, function (examples, ice) {
        ice.loop('exampleDoc', examples, function (i, example, ice) {
          var parsed = (0, _utilJs.parseExample)(example);
      ...
      ice.into('reactPropTypes', doc.reactPropTypes, function (reactPropTypes, ice) {
        var exampleFragment = getReactPropTypesExample(doc.name, reactPropTypes);
        ice.text('reactExampleCode', exampleFragment);
        ice.loop('reactPropType', reactPropTypes, function (i, reactPropType, ice) {
    ...
    value: function _buildExpressionExtendsHTML(doc) {
      ...
      var html = doc.expressionExtends.replace(/[A-Z_$][a-zA-Z0-9_$]*/g, function (v) {
        return _this2._buildDocLinkHTML(v);
      });
