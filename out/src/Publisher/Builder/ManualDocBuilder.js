...
var ManualDocBuilder = (function (_DocBuilder) {
  ...
  _createClass(ManualDocBuilder, [{
    ...
    value: function _buildManualNav(manualConfig) {
      ...
      var ice = new _iceCap2['default'](this._readTemplate('manualNav.html'));
      ice.loop('navItem', manualConfig, function (i, item, ice) {
        ice.text('link', item.label);
        ice.attr('link', 'href', _this._getManualOutputFileName(item));
      });
      return ice;
    }
    ...
    value: function _convertMDToHTML(item) {
      var content = _fsExtra2['default'].readFileSync(item.path).toString();
      var html = (0, _utilJs.markdown)(content);
      var $root = _cheerio2['default'].load(html).root();
      var $h1 = $root.find('h1');
      var synonyms = this._getLabelSynonyms(item.label);
